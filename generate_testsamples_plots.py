...
def load_model():
    ...
    model.load_state_dict(torch.load('models/DDPM_downscale_' + "v2_fixeddata" + '/ckpt.pt',
                                             map_location=device))

    model2 = UNet_downscale(c_in = args.c_in, c_out = args.c_out,
                                   img_size = args.image_size,
                                   interp_mode=args.interp_mode, device=device).to(device)
    # load the trained model
    model2.load_state_dict(torch.load('models/DDPM_downscale_' + "v3_fixeddata_resratio8" + '/ckpt.pt',
                                             map_location=device))

    model3 = UNet_downscale(c_in = args.c_in, c_out = args.c_out,
                            img_size = args.image_size,
                            interp_mode=args.interp_mode, device=device).to(device)
    # load the trained model
    model3.load_state_dict(torch.load('models/DDPM_downscale_' + "v4_fixeddata_resratio16" + '/ckpt.pt',
                                      map_location=device))
    ...
    for i in range(args.n_example_imgs//4):
        ...
        sampled_images2 = diffusion.sample(model2, n=len(images_lr), images_lr = images_lr, cfg_scale = 0)
        sampled_images3 = diffusion.sample(model3, n=len(images_lr), images_lr = images_lr, cfg_scale = 0)

        # save everything
        for j in range(sampled_images.shape[0]):
            filename = os.path.join("Plotting_" + args.folder_name + "/Image_test_generated", f"{4*i+j}_test_generated.jpg")
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            save_images(sampled_images[j], filename)
            filename = os.path.join("Plotting_" + args.folder_name + "/Image_test_generated_resratio8", f"{4*i+j}_test_generated.jpg")
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            save_images(sampled_images2[j], filename)
            filename = os.path.join("Plotting_" + args.folder_name + "/Image_test_generated_resratio16", f"{4*i+j}_test_generated.jpg")
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            save_images(sampled_images3[j], filename)
            filename = os.path.join("Plotting_" + args.folder_name + "/Image_test_lowers", f"{4*i+j}_test_lowres.jpg")
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            save_images(images_lr_save[j], filename)
            filename = os.path.join("Plotting_" + args.folder_name + "/Image_test_truth", f"{4*i+j}_test_truth.jpg")
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            save_images(images_hr_save[j], filename)
            filename = os.path.join("Plotting_" + args.folder_name + "/Image_test_bicubic", f"{4*i+j}_test_bicubic.jpg")
            os.makedirs(os.path.dirname(filename), exist_ok=True)
