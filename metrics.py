...
if __name__ == '__main__':
    rpath = "./metric_test_true" #ground truth path
    mpath = "./metric_test_result" #result image path
    img_r, size , _= read_image(rpath)
    img_m, _ , _= read_image(mpath)
    
    p=[]
    s=[]
    for i in range(size):
        p.append(psnr(img_r[i], img_m[i]))
        s.append(ssim(img_r[i], img_m[i]))
    p=np.array(p)
    s=np.array(s)
    psnr_val = np.mean(p)
    ssim_val = np.mean(s)
    mse_val = mse(img_r,img_m)
    mae_val = mae(img_r,img_m)
    print(psnr_val)
    print(ssim_val)
    print(mse_val)
    print(mae_val)
